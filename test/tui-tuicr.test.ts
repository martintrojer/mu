...
import { runTuicrInteractive } from "../src/cli/tui/tuicr.js";
...
function enoent(): Error {
  const e = new Error("spawnSync tuicr ENOENT") as NodeJS.ErrnoException;
  e.code = "ENOENT";
  return e;
}

describe("runTuicrInteractive", () => {
  it("runs `tuicr -r REV` in the requested cwd and restores the alt screen on success", () => {
    const writes: string[] = [];
    let call: SpawnCall | null = null;
    const env = { PATH: "/test/bin" } as NodeJS.ProcessEnv;

    const r = runTuicrInteractive(
      { rev: "abc123", cwd: "/tmp/repo" },
      {
        env,
  ...
  it("converts ENOENT into an install hint and still restores the alt screen", () => {
    ...
    expect(r.error).toContain("tuicr not found");
    expect(writes).toEqual([ALT_SCREEN_EXIT, ALT_SCREEN_ENTER]);
  });
  ...
  it("reports non-zero exit statuses", () => {
    ...
    expect(r).toEqual({ ok: false, error: "tuicr exited 17" });
    expect(writes).toEqual([ALT_SCREEN_EXIT, ALT_SCREEN_ENTER]);
  });
