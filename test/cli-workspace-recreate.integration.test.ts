...
describe("recreateWorkspace SDK", () => {
  ...
  it("recreate emits ONE `workspace recreate` event (not free + create)", async () => {
    ...
    const before = db.prepare("SELECT COUNT(*) AS c FROM ops WHERE entity = 'workspace'").get() as {
      c: number;
    };
    ...
    const after = (
      db.prepare("SELECT COUNT(*) AS c FROM ops WHERE entity = 'workspace'").get() as {
        c: number;
      }
    ).c;
    const delta = after - before.c;
    const newEvents = db
      .prepare("SELECT payload FROM ops WHERE entity = 'workspace' ORDER BY seq DESC LIMIT ?")
