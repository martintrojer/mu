...
| `src/state.ts`        | SDK seam for the `mu state` verb. `loadWorkstreamSnapshot(db, ws, opts?)` is the one read pass over the SDK both the static renderer (`src/cli/state.ts`) and the TUI (`src/cli/tui/`) consume. Plus pure derivation helpers: `agentStatusHistogram(agents)`, `summarizeOwnedTasks(owned)`, `roiBucket(impact, effortDays)`. |
