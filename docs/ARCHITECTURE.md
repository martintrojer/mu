...
| `src/shell-quote.ts`  | Shared POSIX single-quote helper (`shellQuote`) for interpolating tokens into copy-pasteable `/bin/sh` next-step hints; consumed by `db-sync.ts` and `db-sync-replay.ts`. |
