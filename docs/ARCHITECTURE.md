...
| `src/logs.ts`         | **Deprecated shim** (v2-log-verb replaces it): keeps the v1 `appendLog` / `listLogs` / `latestSeq` / `emitEvent` signatures but reads and writes `ops` rows underneath, mapping `kind`→`ops.entity`, `source`→`ops.actor`, `workstream`→`ops.key` (`''` = machine-wide). Placeholder `hlc`/`group_id` until v2-hlc / v2-capture own them. |
