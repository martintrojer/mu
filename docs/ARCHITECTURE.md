...
| `src/undo.ts`         | **Undo as inverse ops.** v1 swapped the whole DB file back to a snapshot, which reverted the operator's OTHER workstreams too and did not sync, so a peer kept the mistake. 2.0 undo emits inverse ops for one `group_id`: granular (only that action's rows), composable (the undo is an ordinary op in its own group, so it syncs AND is itself undoable — that is the whole of "redo", with no separate mechanism), and file-free. Inverses are computed from PROVENANCE, not a flag: a put created the row iff no op for that key precedes it, and a field's prior value is the newest earlier op naming it — the same query `src/apply.ts` uses for per-field LWW, deliberately through the same helper, because a second implementation of "what was this field before" is how the two drift and drift here means undo restoring wrong values. Mutates the tables inside a `withOpContext` scope and lets the capture triggers record the result rather than hand-writing ops, so an undo cannot be the one write path capture misses. Refuses a group SUPERSEDED by later work (exit 4, naming the conflict) rather than silently discarding it; `--force` is the deliberate override. |
...
| `src/parked.ts`       | Read-only "presumed parked on another machine" heuristic behind `mu workstream list`'s `parked` column and the TUI tab strip's dim marker. **Effectively dormant**: it keys on the workstream's latest op being `workstream.export`, which was the closest surviving relative of v1's `db export` marker, and 2.0 removed that verb. Kept keyed on an INTENT rather than a payload prefix so it cannot silently mis-fire; the honest signal is peer **watermarks**, which is what should re-ground it. |
...
| `src/cli/db.ts`       | `mu db backup <file>` — all that survives of v1's `db` namespace. `mu db export / import / replay` (~1500 LOC of manifests, per-workstream drift detection, and divergence sidecars) were deleted in 2.0: sync is ambient over **segments** and recovery is `mu rebuild`. Backup stays because the one use those verbs actually got was "give me one file I can scp", and SQLite implements it in a single `VACUUM INTO`. Deliberately NOT an SDK module — there is no policy to share with a second caller — and it never overwrites, since the only reason to run it (the pre-`scripts/v1-to-v2.ts` copy that `SchemaTooOldError` tells you to take) is one you must not clobber on a retry. |
...
| `src/log-render.ts`   | **The ONE op → prose formatter.** `renderOp` maps an op's `intent` (+ natural `key` + named payload fields) to `{verb, subject, detail}`; `renderOpLine` flattens it; `opSubject` resolves the `mu task show` / `mu agent show` target; `parseOpKey` splits `demo/t1#3` / `demo/a->demo/b`. Pure and colour-free, so `src/cli/format.ts` (picocolors) and `src/cli/tui/**` (ink) share one phrasing — no surface owns prose logic. Exhaustive over a closed intent union: a missing case is a COMPILE error, not a runtime fallback. **Never string-matches a payload to decide what an op is** — that was v1's brittleness. Unknown intents (from a newer peer) degrade legibly rather than throwing, so a rendering gap can never block sync. |
...
6. **Records ops** — for portable tables the capture triggers in
   `src/capture.ts` do this inside the SAME transaction, so it
   cannot be forgotten; machine-local changes (`agent.*`,
   `workspace.*`) go through `emitEvent`, which is the one write
   path triggers cannot cover. `mu log --tail` subscribers see the
   new ops on the next 1-second poll.
...
| New schema migration| Bump `CURRENT_SCHEMA_VERSION` in `src/db.ts`; mirror the new shape in `CURRENT_SCHEMA`. In-place bumps work when the change is additive (CREATE-TABLE-IF-NOT-EXISTS picks it up) or destructive-but-idempotent (a version-gated `DROP TABLE IF EXISTS` block). The v8 → v9 2.0 bump did neither: it is a **clean break with no migration path**, because a major version is exactly the moment to stop carrying one. `openDb` rejects pre-v9 DBs with `SchemaTooOldError` (exit 4). Operators with v8 data run the SIDECAR `scripts/v1-to-v2.ts` importer once, by hand, against a copy: it opens the v8 file READ-ONLY and synthesizes one `put` op per existing row, HLC-ordered by the source `created_at`, under one synthetic group with intent `migrate.v1`. **Ops, not rows** — v9's tables are a projection of the log, so a direct INSERT would be invisible to sync and reported as drift. Unlike `scripts/migrate-v4-to-v5.ts` (deleted post-landing per the temp-impl-artifact rule) this one is KEPT, because it crosses a major version that users cross on their own schedule. Recipe + the honest not-carried list: [scripts/README.md](../scripts/README.md). |
...
- **Writes go through the typed SDK functions** (`src/agents.ts`,
  `src/tasks.ts`, etc.) which validate, transact, and reconcile. Op
  capture is not their job: the triggers in `src/capture.ts` record
  every portable-table mutation in the SAME transaction, so a
  forgotten call site cannot lose history.
...
- **The ops log is the insurance, and it IS version history.** 2.0
  replaced v1's pre-destructive-verb whole-DB snapshots with one
  append-only `ops` table capturing every change, destructive or
  not. `mu undo <group>` reverts one action by emitting inverse ops;
  `mu rebuild <file>` replays the whole log into a fresh DB.
...
| the whole sync path, as a SESSION | `test/sync-session.integration.test.ts` simulates several days of laptop↔devserver work on ONE workstream: 8 rounds, two machines, no coordination beyond the shared folder, interleaving adds on both sides, cross-machine edges, notes accumulating both ways, close-here/reopen-there, delete-vs-concurrent-edit in BOTH hlc orders, claim/release, an archive marker restored on the other machine, and an undo propagating. Exists because the 28 tests in `test/cli-sync.integration.test.ts` are all SINGLE-ROUND (create, diverge once, sync once, assert) and so cannot express "an op arrives before the op it depends on" — which is the case that found the `reprojectDeferredOps` bug. Five end-state assertions: byte-identical portable content on both DBs (surrogate ids and `owner_id` excluded, since two converged machines are SUPPOSED to disagree about ownership), no drift under `mu doctor --deep` on both, `mu rebuild` of either reproducing the same state, `--repair` from zero changing nothing (the strongest idempotence check available), and ops NOT growing superlinearly — with a final quiet-rounds check that an exchange with no new work adds exactly zero ops, which is the echo-loop detector in its sharpest form. The ordering case is a separate describe block that RENAMES segments to pin ingest order, because otherwise it passes or fails at random (measured 5-of-8 before the fix) |
