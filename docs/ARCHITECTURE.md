...
- **Human entrypoint:** bare `mu` launches the read-only TUI when
  ...
  machine and uses `$MU_SESSION` only to choose the initial active tab
  when the env var names one of those loaded workstreams. If no
...
| `src/vcs.ts`          | `VcsBackend` interface + jj / sl / git / none impls; detection precedence; `commitsBehind(workspacePath, ref)` for staleness signal (no auto-fetch; pure observation); `recentCommits(projectRoot, limit)` + `showCommit(projectRoot, sha)` for the TUI's project-root Commits card/popup; `isClean(workspacePath)` cheap working-copy probe used by `closeAgent`'s clean-workspace auto-free path |
