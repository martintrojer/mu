...
| **bucket**            | The `--out` directory of an **export**: a multi-source tree (bucket-wide `INDEX.md` + one subdir per source) that `mu workstream export` and `mu archive export` both write, additively. Read-only artifact. | "output dir", "archive" (an archive is markers) |
