...
| **log entry**         | A rendered **op**, as shown by `mu log`. Not a distinct table — the ops log is the only log, and since v2-retire-log-shim `mu log` really does read every op rather than a prose subset of them. | "message" (overloaded), "event" (overloaded)       |
| **kind**              | The `--kind <tag>` on a **log entry**, which is the op's `entity`. Read-side `mu log --kind <tag>` filters to it. Captured ops use the entity they mutate (`task`, `note`, `edge`, `workstream`); ops for machine-local changes no trigger can see use `agent` / `workspace`. `message` is the default for operator-authored writes. The v1 catch-all `event` is retired: a prose row with no **intent** could only be rendered by prefix-matching its text. | "category", "type" (overloaded) |
